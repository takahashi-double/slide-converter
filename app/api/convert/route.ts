...
export async function POST(req: NextRequest) {
  ...
  if (!file) return new Response(JSON.stringify({ error: 'ファイルが見つかりません' }), { status: 400 });
  if (file.size > 4 * 1024 * 1024) return new Response(JSON.stringify({ error: '4MB以下にしてください' }), { status: 400 });
